...
def load_dataset(tokenizer, train_batch_size, test_batch_size, model_name, method_name, workers):
    data = pd.read_csv('./data/pingjia1_train.csv', sep=',', encoding='utf-8')
    len1 = int(len(list(data['label'])) * 0.1)
    labels = list(data['label'])#[0:len1]
    sentences = list(data['text_a'])#[0:len1]
    # labels = list(data['label'])
    # sentences = list(data['text_a'])
